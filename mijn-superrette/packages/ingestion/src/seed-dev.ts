...
export async function countDevelopmentRows(db: Database): Promise<number> {
  const r = await db.execute<{ n: number }>(
    sql`SELECT count(*)::int AS n FROM catalog.retailer_products WHERE data_origin = 'DEVELOPMENT_SEED'`,
  );
  return r.rows[0]?.n ?? 0;
}
...
export async function seedDevelopmentData(
...
): Promise<{ sync: SyncReport; equivalences: number }> {
  ...
  const registry = createDefaultRegistry({
    ...
    developmentSeed: { ...(options.now ? { now: options.now } : {}) },
  });
  ...
  log(
    `Development seed sync ${sync.status}: read ${sync.read}, created ${sync.created}, updated ${sync.updated}, failed ${sync.failed}.`,
  );
  ...
  if (!sub) {
    await db.insert(subscriptions).values({
      ...
      store: 'MANUAL',
      status: 'ACTIVE',
      originalTransactionId: `dev-${demoId}`,
    });
  }
    ...
    for (const slug of ['colruyt', 'delhaize', 'albert-heijn', 'lidl', 'jumbo']) {
      ...
      if (r)
        await db
          ...
          .values({ userId, retailerId: r.id, hasLoyaltyCard: slug === 'colruyt' })
          .onConflictDoNothing();
    }
  }
  ...
  const [list] = await db
    ...
    .from(shoppingLists)
    .where(and(eq(shoppingLists.ownerId, demoId), eq(shoppingLists.name, 'Weekboodschappen')));
  if (!list) {
    const [created] = await db
      .insert(shoppingLists)
      .values({ ownerId: demoId, name: 'Weekboodschappen', kind: 'weekly' })
  ...
  for (const name of favoriteNames) {
    const [v] = await db
      ...
      .where(eq(productVariants.displayName, name));
