...
describe.skipIf(!process.env.INTEGRATION)('worker with BullMQ', () => {
  it('processes a queued provider sync', async () => {
    ...
    try {
      const job = await worker.queues.sync.add('test', {
        syncId: '',
        providerKey: 'development-seed',
        kind: 'PROMOTIONS',
        triggeredBy: 'test',
      });
      ...
      for (let i = 0; i < 60 && status !== 'SUCCESS' && status !== 'FAILED'; i++) {
        ...
        const rows = await db.db.execute<{ status: string }>(
          sql`SELECT s.status FROM ingest.provider_syncs s JOIN ingest.import_jobs j ON j.sync_id = s.id WHERE j.external_job_id = ${job.id}`,
        );
