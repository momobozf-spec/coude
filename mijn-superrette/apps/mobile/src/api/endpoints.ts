...
export function endpoints(c: ApiClient) {
  return {
    ...
    plans: () => c.get<{ key: string; name: string; isDefault: boolean; entitlements: { key: string; enabled: boolean; limit: number | null }[] }[]>('/v1/plans', { auth: false }),
    ...
    search: (p: SearchParams) => c.get<SearchResponse>(`/v1/search${qs({ q: p.q, sort: p.sort, promotionOnly: p.promotionOnly, retailers: p.retailers })}`),
    ...
    product: (id: string, scope: 'mine' | 'all' = 'mine') => c.get<ProductDetailDto>(`/v1/products/${id}${qs({ scope })}`),
    ...
    addItem: (listId: string, input: Partial<CreateListItemInput> & { title: string }) => c.post<ShoppingListItemDto>(`/v1/lists/${listId}/items`, input),
    updateItem: (listId: string, itemId: string, input: UpdateListItemInput) => c.patch<ShoppingListItemDto>(`/v1/lists/${listId}/items/${itemId}`, input),
    ...
    compare: (listId: string, retailerIds?: string[]) => c.post<BasketComparisonDto>(`/v1/lists/${listId}/compare`, retailerIds ? { retailerIds } : {}),
    smartBasket: (listId: string, input: Partial<SmartBasketInput>) => c.post<SmartBasketDto>(`/v1/lists/${listId}/smart-basket`, input),
    activity: (listId: string) => c.get<ListActivityDto[]>(`/v1/lists/${listId}/activity`),
    createInvite: (listId: string, role: 'EDITOR' | 'VIEWER' = 'EDITOR') => c.post<InviteDto>(`/v1/lists/${listId}/invites`, { role }),
    ...
    promotions: (params: { section?: string; sort?: string; retailerId?: string; category?: string }) => c.get<PromotionDto[]>(`/v1/promotions${qs(params)}`),

    alerts: () => c.get<PriceAlertDto[]>('/v1/alerts'),
    createAlert: (input: CreateAlertInput) => c.post<PriceAlertDto>('/v1/alerts', input),
    updateAlert: (id: string, input: { enabled?: boolean; targetPriceCents?: number | null }) => c.patch<PriceAlertDto>(`/v1/alerts/${id}`, input),
    ...
    registerPushToken: (token: string, platform: 'ios' | 'android' | 'web') => c.post<void>('/v1/push-tokens', { token, platform }),
  };
}
