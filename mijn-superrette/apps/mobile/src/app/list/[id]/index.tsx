...
function ItemRow({
  ...
  return (
    <Row style={{ paddingVertical: 10 }} gap={12}>
      ...
      <Pressable
        style={{ flex: 1 }}
        onPress={() => item.preferredVariantId && router.push(`/product/${item.preferredVariantId}`)}
      >
...
export default function ListScreen(): ReactNode {
  ...
  const toggle = useMutation({
    mutationFn: (item: ShoppingListItemDto) =>
      api.updateItem(id, item.id, { checked: !item.checked, version: item.version }),
    onMutate: (item) =>
      qc.setQueryData<ShoppingListDetailDto>(key, (prev) =>
        prev ? { ...prev, items: prev.items.map((i) => (i.id === item.id ? { ...i, checked: !i.checked } : i)) } : prev,
      ),
    onSuccess: (updated) =>
      qc.setQueryData<ShoppingListDetailDto>(key, (prev) =>
  ...
  const activityText = lastActivity
    ? {
        ITEM_ADDED: t('lists.activityAdded', {
          name: lastActivity.userDisplayName ?? '?',
          item: String(lastActivity.payload.item ?? ''),
        }),
        ITEM_CHECKED: t('lists.activityChecked', {
          name: lastActivity.userDisplayName ?? '?',
          item: String(lastActivity.payload.item ?? ''),
        }),
        ITEM_UNCHECKED: t('lists.activityUnchecked', {
          name: lastActivity.userDisplayName ?? '?',
          item: String(lastActivity.payload.item ?? ''),
        }),
        ITEM_REMOVED: t('lists.activityRemoved', {
          name: lastActivity.userDisplayName ?? '?',
          item: String(lastActivity.payload.item ?? ''),
        }),
  ...
  return (
    <Screen
      ...
      right={
        l && l.role === 'OWNER' ? (
          <IconButton icon="share" label={t('lists.share')} onPress={() => router.push(`/list/${id}/share`)} />
        ) : null
      }
        ...
        <>
          ...
          {l.role !== 'VIEWER' ? (
            <SearchField
              ...
              onSubmit={() => text.trim() && add.mutate(text.trim())}
            />
          ) : null}
          ...
          {l.items.length > 0 ? (
            <View style={{ gap: 10, marginTop: 20 }}>
              <Button
                title={t('lists.compare')}
                icon="basket"
                size="lg"
                onPress={() => router.push(`/list/${id}/compare`)}
              />
              <Button
                title={t('lists.smartBasket')}
                icon="sparkle"
                variant="accent"
                size="lg"
                onPress={() => router.push(`/list/${id}/smart`)}
              />
            </View>
          ) : null}
        </>
