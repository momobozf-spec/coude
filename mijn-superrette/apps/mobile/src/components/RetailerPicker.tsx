...
export function RetailerPicker({ country, onSaved, cta }: { country: string; onSaved: () => void; cta: string }): ReactNode {
  const api = useApi();
  const { t } = useI18n();
  ...
  const save = async (): Promise<void> => {
    setBusy(true);
    try {
      await api.setMyRetailers({ retailers: chosen.map((r) => ({ retailerId: r.id, hasLoyaltyCard: Boolean(cards[r.id]) })) });
      onSaved();
    } finally {
      setBusy(false);
    }
    ...
    <View style={{ gap: 12 }}>
      <Row gap={8} style={{ flexWrap: 'wrap' }}>
        {retailers.data?.map((r) => (
          <Chip key={r.id} label={r.name} color={r.brandColor} selected={Boolean(selected[r.id])} onPress={() => setSelected((s) => ({ ...s, [r.id]: !s[r.id] }))} />
        ))}
      </Row>
      {chosen.some((r) => r.loyaltyProgramName) ? (
        <Card style={{ marginTop: 12 }}>
          ...
          {chosen
            ...
            .map((r) => (
              <ToggleRow key={r.id} label={`${r.name} · ${r.loyaltyProgramName}`} value={Boolean(cards[r.id])} onValueChange={(v) => setCards((c) => ({ ...c, [r.id]: v }))} />
            ))}
        </Card>
      ) : null}
      <Button title={cta} onPress={save} loading={busy} disabled={chosen.length === 0} size="lg" style={{ marginTop: 16 }} />
    </View>
  );
}
