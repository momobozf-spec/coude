...
describe('Vertical slice 1: search → compare → favourite → shopping list', () => {
  it('works end to end for a newly onboarded user', async () => {
    ...
    await client
      .put('/v1/me/retailers', { retailers: Object.values(ids).map((retailerId) => ({ retailerId, hasLoyaltyCard: false })) })
      .expect(200);
    ...
    expect(names).toEqual(expect.arrayContaining(['Boni Halfvolle Melk 1 l', 'AH Halfvolle melk 1 l', 'Milbona Halfvolle melk 1 l']));
    ...
    expect(byUnit.body.items[0]).toMatchObject({ name: 'Milbona Halfvolle melk 1 l', cheapest: { retailerName: 'Lidl', priceCents: 99, unitPrice: { cents: 99, per: 'l' } } });
    ...
    expect(detail.cheapest).toMatchObject({ retailer: { slug: 'colruyt' }, priceCents: 105, unitPrice: { cents: 105, per: 'l' }, isCheapest: true });
    const equivalents = (await client.get(`/v1/products/${boni.variantId}/equivalents`).expect(200)).body as EquivalentDto[];
    expect(equivalents.map((e) => e.name)).toEqual(expect.arrayContaining(['AH Halfvolle melk 1 l', 'Milbona Halfvolle melk 1 l', 'Delhaize Lait demi-écrémé / Halfvolle melk 1 l']));
    ...
    const colaDetail = (await client.get(`/v1/products/${cola.items[0]!.variantId}?scope=all`).expect(200)).body as ProductDetailDto;
    expect(colaDetail.name).toBe('Coca-Cola Zero Sugar 1,5 l');
    expect(colaDetail.offers.length).toBe(7);
    expect(colaDetail.cheapest).toMatchObject({ retailer: { slug: 'dirk' }, priceCents: 199, isPromotion: true, unitPrice: { cents: 133, per: 'l' } });
    ...
    expect(ah.missedPromotions[0]).toMatchObject({ label: '2 voor €4', reason: 'NO_BENEFIT_AT_QUANTITY', minimumQuantity: 2 });
    ...
    const list = (await client.post('/v1/lists', { name: 'Weekboodschappen', kind: 'weekly' }).expect(201)).body as ShoppingListDto;
    await client.post(`/v1/lists/${list.id}/items`, { title: detail.name, preferredVariantId: boni.variantId, quantity: 2 }).expect(201);
    const listDetail = (await client.get(`/v1/lists/${list.id}`).expect(200)).body as ShoppingListDetailDto;
    expect(listDetail.items[0]).toMatchObject({ preferredVariantId: boni.variantId, quantity: 2, preferredVariantName: 'Boni Halfvolle Melk 1 l' });
...
describe('Vertical slice 2: compare a list, review matches, change an equivalent', () => {
  it('recalculates totals when the user changes a product', async () => {
    ...
    await client.put('/v1/me/retailers', { retailers: Object.values(ids).map((retailerId) => ({ retailerId, hasLoyaltyCard: false })) }).expect(200);
    const list = (await client.post('/v1/lists', { name: 'Deze week', kind: 'thisWeek' }).expect(201)).body as ShoppingListDto;
    for (const title of ['Melk', 'Brood', 'Eieren']) await client.post(`/v1/lists/${list.id}/items`, { title }).expect(201);
    ...
    await client.put(`/v1/lists/${list.id}/items/${milk.itemId}/selection`, { retailerId: colruyt.retailer.id, retailerProductId: arla.retailerProductId }).expect(204);
    ...
    // Reset the choice.
    await client.put(`/v1/lists/${list.id}/items/${milk.itemId}/selection`, { retailerId: colruyt.retailer.id, retailerProductId: null }).expect(204);
...
describe('Vertical slice 3: price alert → new observation → push notification', () => {
  it('triggers once and never duplicates', async () => {
    ...
    const alert = (await demo.post('/v1/alerts', { variantId, targetPriceCents: 200 }).expect(201)).body as PriceAlertDto;
    ...
    const imported = await admin.post('/v1/admin/dev/price-observations', { retailerProductId: colruytOffer.retailerProductId, regularPriceCents: 219, promoPriceCents: 189 }).expect(201);
    expect(imported.body.stored).toBe(true);
    expect(imported.body.alerts.triggered).toBeGreaterThanOrEqual(1);

    const notifications = (await demo.get('/v1/notifications').expect(200)).body as { items: NotificationDto[]; unread: number };
    ...
    expect(push.sent.some((m) => m.to === 'ExponentPushToken[test-device-demo]' && m.title === 'Prijsalarm!')).toBe(true);

    // The same observation again: no duplicate notification.
    const again = await admin.post('/v1/admin/dev/price-observations', { retailerProductId: colruytOffer.retailerProductId, regularPriceCents: 219, promoPriceCents: 189 }).expect(201);
    ...
    await admin.post('/v1/admin/dev/price-observations', { retailerProductId: colruytOffer.retailerProductId, regularPriceCents: 219, promoPriceCents: null }).expect(201);
    ...
    await new Client(app, admin.token).post('/v1/admin/dev/price-observations', { retailerProductId: colruytOffer.retailerProductId, regularPriceCents: 219, promoPriceCents: 179 }).expect(201);
