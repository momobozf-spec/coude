...
export class NotificationsController {
  ...
  async register(
    ...
    @Body(new ZodPipe(pushTokenSchema)) body: { token: string; platform: 'ios' | 'android' | 'web' },
  ): Promise<void> {
    await this.db
      .insert(pushTokens)
      ...
      .onConflictDoUpdate({
        ...
        set: { userId: user.id, platform: body.platform, disabledAt: null, lastUsedAt: new Date() },
      });
  }
